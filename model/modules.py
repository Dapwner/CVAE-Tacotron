...
class AccentEncoderNetwork(nn.Module):
    def __init__(self, model_config):
        ...
        self.cvae = CVAEnet(model_config["reference_encoder"]["ref_enc_gru_size"],model_config["reference_encoder"]["ref_enc_gru_size"],model_config["accent_encoder"]["y_dim"],model_config["speaker_encoder"]["y_dim"],model_config["accent_encoder"]["z_dim"],model_config["speaker_encoder"]["z_dim"],model_config["accent_encoder"]["n_classes"],model_config["speaker_encoder"]["n_classes"])
    ...
    def forward(self, inputs, acc_labels=None, spk_labels=None, input_lengths=None):
        ...
        (z_acc, y_acc, z_spk, y_spk, (mu_acc, var_acc, mu_spk, var_spk)) = self.cvae(enc_out, acc_labels, spk_labels)

        # print(out['prob_cat'].shape, out['logits'].shape)

        return (z_acc, y_acc, z_spk, y_spk, (mu_acc, var_acc, mu_spk, var_spk))
...
#        (z, (mu, var, y_mu, y_var, logits)) = self.mlvae.inference(enc_out,labels)
        ...
        (z_acc, y_acc, z_spk, y_spk, (mu_acc, var_acc, mu_spk, var_spk)) = self.cvae.inference(enc_out, acc_labels, spk_labels)
